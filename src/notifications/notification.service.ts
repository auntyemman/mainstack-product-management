...
export class NotificationService {
  ...
  async markNotificationAsRead(notificationId: string): Promise<INotification>{
    const update = await this.notificationRepository.update(notificationId, { isRead: true });
    if (!update) {
      throw new UnprocessableEntityError('Failed to mark notification as read');
    }
    return update;
  }
}
